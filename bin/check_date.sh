...
from ori.label_doss
" > mark_doss_tmp
cat /home/ori/general/bin/conf/mark_doss |awk -F "\t" '{print $1"\t"$2}' >>mark_doss_tmp
...
from flow_tmp) t1 left join (select distinct trim(sexual) as sexual from ori.sexual) t2
...
from flow_tmp) t1 left join (select distinct trim(province) as province from ori.province) t2
...
from flow_tmp) t1 left join (select distinct trim(city) as city from ori.city) t2
on t1.city=t2.city
...
from flow_tmp) t1 left join (select distinct trim(name) as name from ori.name) t2
on t1.name=t2.name
...
from flow_tmp) t1 left join (select distinct trim(primary_classification) as primary_classification from ori.primary_classification) t2
...
from flow_tmp) t1 left join (select distinct trim(second_level_classification) as second_level_classification from ori.second_level_classification) t2
